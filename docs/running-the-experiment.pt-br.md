...
Duas verificações ao copiar uma suíte:

- O caminho de importação é `../src/number-validator` (o mesmo caminho que
  resolve tanto no bench quanto no sandbox de pontuação).
- A suíte passa contra o código correto — a **pré-condição**. O `score.ts`
  sinaliza e ignora qualquer suíte que falhe nela (uma suíte que falha no código
  correto teria falsos positivos, ou seja, precisão < 1).
...
- **P** — precisão: `1` por construção (a pré-condição rejeita falsos
  positivos).
- **F1** — `2PR/(P+R)`, que se reduz a `2R/(1+R)`.
